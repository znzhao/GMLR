...
class PanelGenerator:
    def __init__(self, nX=3, ny=2, ngroup=2, nobs=200, Xrange=(-1, 1), gammarange=(-1, 1), betarange=(-1, 1), sigmarange=(0.1, 0.5)):
        '''
        ...
        self.X = pd.DataFrame(np.random.rand(nobs, nX), columns=self.Xcol)
        ...
        self.y = pd.DataFrame(self.simulate(), columns=self.ycol)
        # Combine the independent and dependent variables into a single data frame
        self.data = pd.concat([self.X[self.Xcol], self.y[self.ycol]], axis=1)
    ...
    def simulate(self):
        ...
        return np.vstack(ys)  # Return the y values as a numpy array
...
if __name__ == "__main__":
    paneldata = PanelGenerator()  # Create an instance of the PanelGenerator
