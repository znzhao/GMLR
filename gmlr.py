...
class GMLR:
    ...
    def filter(self, X: np.array, y: np.array, thetas: np.array):
        """
    ...
    def modelFit(self, inputX, inputy, maxiter = 100, tol = 1e-4, disp = True, plot = True, plotx = None, ploty = None):
        ...
        for stepi in range(maxiter):
            postiors = self.filter(self.X, self.y, thetas)
    ...
    def deltaMethod(self, thetas: np.array, varthetas: np.array):
        ...
        - BIC value.
        """
        delta = gradient(thetas, lambda thetas: self.__unpack(thetas)[2])
        if self.cov:
    ...
    def __predFunc(self, xi: int, X: np.array, thetas: np.array):
    ...
    def predict(self, X: np.array = None, disp = True, lb = 0.05, ub = 0.95):
        """
        Generates predictions using the trained model.

        Input:
        - X: Numpy array of independent variables (optional, default is None, which uses the training data).
        ...
        with Timer('Model Prediction'):
            for xi in tqdm.trange(X.shape[0], disable = not disp, desc = 'Model Prediction', leave = False):
                yi = self.__predFunc(xi, X, self.thetas)
                gradyi = gradient(self.thetas, lambda thetas: self.__predFunc(xi, X, thetas))
    ...
    def plot(self, thetas, x, y, ax = None, figsize = (16,8), truelabel = None, show = True):
        ...
        ls = []
        for g in range(self.ngroup):
            ax.scatter(self.data[x][np.argmax(self.filter(self.X, self.y, thetas), axis=1) == g], 
                   self.data[y][np.argmax(self.filter(self.X, self.y, thetas), axis=1) == g], 
    ...
    def plotMSE(self, y, figsize = (16,8), showci = True, alpha = 0.1):
        """
...
if __name__ == "__main__":
    ...
    gmlr = GMLR(data.data,  ycol=data.ycol, Xcol=data.Xcol, alpha=0,  ngroup=2, cov=False)
